...
export default function CustomerPage() {
    ...
    const fetchCarList = async () => {
      try {
        ...
        if (data && Array.isArray(data.cars)) {
          ...
          console.error('Dữ liệu xe không được trả về dưới dạng mảng:', data);
  ...
  const handleAddCustomer = async () => {
    try {
      ...
      if (!customerData.nameCar || customerData.nameCar.trim() === '') {
        errors.nameCar = 'Vui lòng nhập xe thuê của Khách Hàng.';
      }
  ...
  const handleEditCustomer = async (customerId) => {
    try {
      ...
      const response = await axios.put(`http://localhost:5000/update-customer/${customerId}`, formData, {
        headers: {
        ...
        <Dialog open={openAddDialog} onClose={handleCloseAddDialog}>
          ...
          <DialogContent>
            <Grid container spacing={2}>
              ...
              <Grid item xs={12}>
                <InputLabel htmlFor="imagePath">Xe Thuê</InputLabel>
                <Select
                  fullWidth
                  name="nameCar"
                ...
                <TableBody>
                  {filteredUsers.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((customer) => (
                    <TableRow hover key={customer._id} tabIndex={-1} role="checkbox">
                      ...
                      <TableCell align="left">{customer.title}</TableCell>
